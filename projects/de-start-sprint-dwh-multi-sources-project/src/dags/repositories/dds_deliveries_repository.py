...
from models import DeliveryObj
...
class DestRepository:
    def insert(self, conn: Connection, object: DeliveryObj,
               query: str) -> None:
        with conn.cursor() as cur:
            cur.execute(
                query,
                {
                    ...
                    "delivery_ts_id": object.delivery_ts_id,
...
# Преобразуем к строке, чтобы положить в БД.
    ...
    def __init__(self, pg_conn: PgConnect, log: Logger) -> None:
        ...
        self.origin = OriginRepository[DeliveryObj](pg_conn, DeliveryObj)
    ...
    def load(self):
        with self.pg_dest.connection() as conn:
            ...
            query = self.sql_dir.load_sql_file("stg_get_deliveries.sql")
            ...
            query = self.sql_dir.load_sql_file("dds_insert_deliveries.sql")
