...
def project_spr12_dag_get_data():
    fetch_task = PythonOperator(
        task_id=f"fetch_{source_file.replace('.csv', '')}",
    ...
    load_task = PythonOperator(
        task_id=f"load_{source_file.replace('.csv', '')}",
        python_callable=load_stg,
        op_kwargs={'file_name': source_file}
    )

    print_10_lines = BashOperator(
        task_id='print_10_lines',
        bash_command=bash_command_tmpl,
        params={'files': f'/data/{source_file}'}
    )

    fetch_task >> print_10_lines >> load_task


_ = project_spr12_dag_get_data()
